    ...
    return (
        <div className="min-h-[70vh] flex items-center justify-center bg-aura-cream/30 py-12">
            <div className="bg-white p-8 md:p-12 shadow-sm border border-gray-100 max-w-md w-full">
                <h1 className="text-3xl font-serif text-center text-aura-dark mb-8">Join Aura</h1>
                {error && <div className="bg-red-50 text-red-500 p-3 text-sm mb-6">{error}</div>}

                <form onSubmit={handleSubmit} className="space-y-6">
                    <div>
                        <label className="block text-xs uppercase tracking-wider text-aura-text mb-2">Full Name</label>
                        <input
                            type="text"
                            required
                ...
                <p className="text-center mt-6 text-sm text-aura-lightText">
                    Already have an account? <Link to="/login" className="text-aura-pink hover:underline">Sign In</Link>
                </p>
            </div>
        </div>
    );
